...
# Druphpet
$pimpmylog      = hiera_hash('pimpmylog', {})
$vsftpd         = hiera_hash('vsftpd', {})
$phpmyadmin     = hiera_hash('phpmyadmin', {})
$relic          = hiera_hash('newrelic', {})
...
if array_true($relic, 'install') {
  class { '::druphpet_relic':
    relic => $relic,
    apache => $apache,
    nginx => $nginx
  }
...
if array_true($samba, 'install') {
  class { '::druphpet_samba':
    samba => $samba
  }
}
